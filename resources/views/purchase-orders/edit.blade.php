...
<div class="row">
    <div class="col-12">
        <form action="{{ route('web.ordenes-compra.update', $order->id) }}" method="POST" id="orderForm">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-lg-8">
                            ...
                            <div class="row">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select @error('supplier_id') is-invalid @enderror" id="supplier_id" name="supplier_id" required>
                                            ...
                                            @foreach($suppliers as $supplier)
                                                <option value="{{ $supplier->id }}" {{ old('supplier_id', $order->supplierId) == $supplier->id ? 'selected' : '' }}>
                                                    {{ $supplier->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-3">
                                    <div class="mb-3">
                                        ...
                                        <input type="date" class="form-control @error('order_date') is-invalid @enderror" id="order_date" name="order_date" value="{{ old('order_date', $order->orderDate) }}" required>
                                        @error('order_date')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-md-3">
                                    <div class="mb-3">
                                        ...
                                        <input type="date" class="form-control @error('expected_date') is-invalid @enderror" id="expected_date" name="expected_date" value="{{ old('expected_date', $order->expectedDate) }}">
                                        @error('expected_date')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                @if($oldItems && count($oldItems) > 0)
                                    @foreach($oldItems as $index => $item)
                                        <div class="row mb-3 item-row" data-index="{{ $index }}">
                                            ...
                                            <div class="col-md-2">
                                                <input type="number" class="form-control" name="items[{{ $index }}][unit_cost]" value="{{ $item['unit_cost'] ?? '' }}" step="0.01" min="0" required>
                                            </div>
                                            <div class="col-md-2">
                                                <input type="text" class="form-control" name="items[{{ $index }}][supplier_sku]" value="{{ $item['supplier_sku'] ?? '' }}">
                                            </div>
                                            <div class="col-md-2">
                                ...
                                @else
                                    @foreach($items as $index => $item)
                                        <div class="row mb-3 item-row" data-index="{{ $index }}">
                                            ...
                                            <div class="col-md-2">
                                                <input type="number" class="form-control" name="items[{{ $index }}][unit_cost]" value="{{ $item->unitCost }}" step="0.01" min="0" required>
                                            </div>
                                            <div class="col-md-2">
                                                <input type="text" class="form-control" name="items[{{ $index }}][supplier_sku]" value="{{ $item->supplierSku ?? '' }}">
                                            </div>
                                            <div class="col-md-2">
                                                <button type="button" class="btn btn-outline-danger remove-item"><i class="bi bi-trash"></i></button>
                                            </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control" id="tax_amount" name="tax_amount" value="{{ old('tax_amount', $order->taxAmount) }}" step="0.01" min="0">
                            </div>
                            <hr>
                            ...
                            <div class="d-grid gap-2">
                                <a href="{{ route('web.ordenes-compra.show', $order->id) }}" class="btn btn-secondary">Cancelar</a>
...
@push('scripts')
<script>
let itemIndex = {{ max(count($items), count(old('items', [])), 0) }};
